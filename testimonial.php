	...
	function esha_testimonial_add() {
	    ...
	    if (isset($_POST['cf-submitted'])) {
		    ...
		    elseif(empty($_POST["cf-testimonial"])) {$message = 'Testimonial is empty';}
		        ...
		        $wpdb->insert(
		                $table_name, //table
		                array('name' => $name, 'email' => $email, 'phone' => $phone, 'testimonial' => $testimonial), //data
	...
	function esha_testimonial_list(){
		...
		//delete
		   if (isset($_GET['delete']) && isset($_GET['id'])) {
	        ...
	        $rows = $wpdb->get_results("SELECT id,name,email,phone,testimonial from $table_name");
...
if (!class_exists( 'eshaTestimonialWidget' )) {
	class eshaTestimonialWidget extends WP_Widget {
		...
		public function widget( $args, $instance ) {
			...
			global $wpdb;
	        $table_name = $wpdb->prefix . "testimonial";
	        $testi = $wpdb->get_results("SELECT name,testimonial from $table_name ORDER BY RAND() LIMIT 1");
